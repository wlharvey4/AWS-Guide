...
#+begin_src makefile :tangle Makefile :noweb tangle
  ...
  S3SYNC = $(S3) sync --dryrun $(EXCL_INCL) $(SRC) $(DST_OLD) $(REGION) $(GRANTS)
  S3MOVE = $(S3) mv --dryrun --recursive $(DST_OLD) $(DST_NEW) $(REGION) $(GRANTS)
...
#+name:s3synclocal
#+header: :mkdirp yes :shebang "#!/usr/bin/env bash" :noweb yes
#+begin_src sh :tangle tools/s3synclocal
  verpre=$(aws2 s3 ls pinecone-forest.com/aws-guide/)
  ver=${verpre/*PRE /}
  echo "Version: ${ver}"
  aws2 s3 sync \
       aws-guide s3://pinecone-forest.com/aws-guide/${ver} \
       --exclude "*" \
       --include "*.html" \
       --delete \
  ...
  aws2 s3 mv \
       --recursive \
       s3://pinecone-forest.com/aws-guide/${ver} \
       s3://pinecone-forest.com/aws-guide/<<get-file-version()>> \
       --region us-west-2 \
       --grants read=uri=http://acs.amazonaws.com/groups/global/AllUsers \
#+end_src
