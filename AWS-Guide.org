...
[[*Identity and Access Management in Amazon S3][IAM in S3]]
...
#+texinfo:@heading Exporting

This Makefile has the ability to export to TEXI, but the export produces a very
different set of HTML files than when the export is done from within the ORG
file; plus each program requests permission to run, whereas this does not occur
when exporting from within the ORG file.  Thus, the export should always be
done from within the ORG file until this problem can be resolved.
...
I have attempted to solve this problem; see the ~cmprpl~ program, [[*Compare Replace][below]].
...
#+begin_src emacs-lisp :exports results :eval yes :results silent
(org-babel-tangle-file "AWS-Guide.org")
#+end_src

The ~cmprpl~ source code attempts to resolve the issue of identical exported
code having different time stamps, thus defeating the ~aws2 s3 sync~ command.
This program saves a copy of the current ~aws-guide~ directory prior to
generating a new one upon encountering a ~make html~ target.  It then compares
each file in the saved ~aws-guide-old~ directory with the same named file in
the newly-generated ~aws-guide~ directory.  If the files are identical, the old
file is copied with time-stamp intact into the new directory.  If the files are
different, it skips copying and goes to the next file.  By doing this, it is
hoped to preserve the time stamps of unchanged files going forward so that the
~aws2 s3 sync~ command will not upload them.  It is therefore important to
avoid deleting the ~aws-guide-old~ directory for as long as possible.
...
#+begin_src sh :tangle tools/cmprpl
  ...
  [ -d $NEWDIR ] && echo $NEWDIR exists || { echo "ERROR: $NEWDIR does not exist"; exit 1; }
  [ -d $OLDDIR ] && echo $OLDDIR exists || { echo "SKIPPING: $OLDDIR does not exist"; exit 0; }
  ...
  do
      ...
      if [ -e $newfile ]
      then
	  # echo "$newfile exists in both directories"
	  if cmp -s $newfile $oldfile
	  then
	      # echo "$oldfile and $newfile are equal"
	      cp -pf $oldfile $newfile
	  else
	      echo "Keeping $newfile"
	  fi
      else
	  echo "$oldfile does not exist in newfile"
      fi
  done
#+end_src
