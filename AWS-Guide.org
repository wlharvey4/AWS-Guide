...
including the combined operations of ~sync~ and ~mv~.

TODO Invalidate the prior version’s index.html
...
#+begin_src makefile :tangle Makefile :noweb tangle
  ...
  $(TEXI): $(ORG)
	  emacs --batch \
	  --eval '(progn (require '\''ob-shell)(find-file "$(ORG)")(setq org-confirm-babel-evaluate nil) \
	  (require '\''ox-texinfo)(org-export-to-file '\''texinfo "$(TEXI)"))'
...
** Get S3 Version
This program calls the ~aws2 s3 ls~ command on the bucket associated with this
program to receive the name of the current uploaded version.  AWS returns this
“directory” with the prefix of =PRE=, which is stripped and the resulting
version number is echoed.

#+caption:Get S3 Version
#+name:get-s3-version
#+begin_src sh :eval yes
verpre=$(aws2 s3 ls s3://pinecone-forest.com/aws-guide/)
ver=${verpre/*PRE /}
echo "${ver}"
#+end_src
